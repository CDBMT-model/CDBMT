...
def help_f_cut_stop_word(x, stopwords):
    '''
...
def process_review_data(data):
    ...
    for i in trange(len(data)):
        data['reviewText'][i] = help_f_cut_stop_word(data['reviewText'][i], stop_words)
    ...
    for i in trange(len(data)):
        data['reviewText'][i] = cutWord(data['reviewText'][i], mean_review_len)
...
def main():
    ...
    #data_loader = MyData(review_datas, meta_datas, 5, 17, 45, len(temporal_datas), weights = True)
